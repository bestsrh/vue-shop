<template>
  <div>
   ...
   <!-- 图片预览对话框 -->
    ...
    >
    <img :src="previewPath" alt="" class="previewSize">
    </el-dialog>
  </div>
</template>
<script>
  ...
  methods: {
    ...
    beforeTagLeave(activeName, oldactiveName) {
      if (oldactiveName === '0' && this.addForm.goods_cat.length !== 3) {
        this.$message.error('信息没有全部输入，无法进行下一步')
        return false
      }
    },
    // 点击切换功能
    ...
    handleRemove(file) {
      ...
      const index = this.addForm.pics.findIndex(a => a.pic == filePath)
    ...
    add() {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) {
          return this.$message.error('请填写全部数据')
        }
        // 执行添加的业务逻辑
